...
type MapProps = {
  ...
  loading?: boolean;
  /** Callback when map canvas is clicked */
  onClick?: (e: MapLibreGL.MapMouseEvent) => void;
  /** Callback when the map is initialized and loaded */
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const map = new MapLibreGL.Map({
      ...
      localIdeographFontFamily: "'DM Sans', sans-serif, sans-serif",
      transformRequest: (url: string) => {
        if (url && typeof url === "string" && url.includes("cartocdn.com")) {
          const subdomains = ["a", "b", "c", "d"];
          const sub = subdomains[Math.floor(Math.random() * subdomains.length)];
          return {
            url: url.replace(/https:\/\/(basemaps|[abcd]\.basemaps)\.cartocdn\.com/, `https://${sub}.basemaps.cartocdn.com`),
          };
        }
        return { url };
      },
...
> = {
  ...
  /** Colors for cluster circles: [small, medium, large] based on point count (default: ["#3b82f6", "#059669", "#1e3a8a"]) */
