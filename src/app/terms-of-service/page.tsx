...
import { DEFAULT_TERMS_OF_SERVICE } from "@/lib/legalContent";

export default function TermsOfServicePage() {
  const [content, setContent] = useState<string>(DEFAULT_TERMS_OF_SERVICE);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/legal?doc=terms_of_service")
      .then((res) => res.json())
      .then((data) => {
        if (data.content) {
        ...
        <div className="bg-white rounded-3xl p-8 sm:p-12 shadow-xl border border-black/5">
          ...
          <div className="border-t border-slate-100 mt-12 pt-6 flex flex-wrap items-center justify-between gap-4 text-xs text-slate-500">
            ...
            <div className="flex gap-4">
              <Link href="/privacy-policy" className="text-[#059669] font-bold hover:underline">
                Privacy Policy →
              </Link>
            </div>
          </div>
